...
from ..auxiliar._image_read import read_envi, read_netcdf, show_reflectance_img
from ..auxiliar._logger_class import Logger
from ..auxiliar._spectra_interpolation import spline_interpolation
from ._mlra_gpr import MLRA_GPR
...
class Retrieval:
    ...
    def bio_retrieval(self) -> bool:
        ...
        if self.plot:
            ...
            logging.info("Image not displayed.")
            pass
        ...
        logging.info(f"Getting model {self.number_of_models} names was successful.")
        ...
        # Reading the models
        def import_and_log_model(model_file, bio_models, logger):
            # Importing model
        ...
        # Assuming self.bio_models and self.logger are defined
        list(
            map(
                lambda model_file: import_and_log_model(
                    model_file, self.bio_models, self.logger
                ),
                list_of_models,
            )
        )
        ...
        def run_model(i):
            ...
            logging.info(f"Bands selected. Shape: {data_refl_new.shape}")
            logging.info(f"Elapsed time: {self.process_time}")
            self.logger.log_message(
    ...
    def band_selection(self, i: int) -> np.ndarray:
        ...
        if len(np.intersect1d(current_wl, expected_wl)) == len(expected_wl):
            ...
            logging.info("No matching bands found, spline interpolation is applied.")
            self.logger.log_message(
    ...
    def export_retrieval(self) -> bool:
        ...
        self.logger.log_message("Show images")
        self.show_results()
    ...
    def export_netcdf(self):
        ...
        # Define dimensions
        Nc = self.variable_maps[0].shape[0]
        Nl = self.variable_maps[0].shape[1]
        nc_file.createDimension("Nc", Nc)
        nc_file.createDimension("Nl", Nl)
        ...
        logging.info(f"NetCDF file created successfully at: {self.output_file}")
        self.logger.log_message(
    ...
    def show_results(self):
        ...
        if self.plot:
            ...
            logging.info("Results not displayed.")
            pass
